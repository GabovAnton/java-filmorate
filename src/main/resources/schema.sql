...
VALUES(SELECT ID FROM PUBLIC."filmorate.genres"  WHERE NAME ='DRAMA',
          ...
          (NAME, DESCRIPTION, DURATION, RELEASE_DATE,RATING)
              VALUES('Титаник', 'Грустная история', 90, '2006-01-20','PG13')));
...
VALUES(SELECT ID FROM PUBLIC."filmorate.genres"  WHERE NAME ='CARTOON',
          SELECT ID FROM FINAL TABLE(INSERT INTO PUBLIC."filmorate.films"
          (NAME, DESCRIPTION, DURATION, RELEASE_DATE,RATING)
              VALUES('Босс-молокосос', 'детский мультик про ребенка из корпорации детей', 100, '2008-02-23','G')));
...
VALUES(SELECT ID FROM PUBLIC."filmorate.genres"  WHERE NAME ='ACTION',
          SELECT ID FROM FINAL TABLE(INSERT INTO PUBLIC."filmorate.films"
          (NAME, DESCRIPTION, DURATION, RELEASE_DATE,RATING)
              VALUES('Агент 007', 'экшн про секретного агента', 120, '2020-05-17','PG')));
...
VALUES(SELECT ID FROM PUBLIC."filmorate.genres"  WHERE NAME ='DOCUMENTARY',
          SELECT ID FROM FINAL TABLE(INSERT INTO PUBLIC."filmorate.films"
          (NAME, DESCRIPTION, DURATION, RELEASE_DATE,RATING)
              VALUES('Дикая природа Амазонки', 'документальный фильм про природу', 120, '2025-02-25','G')));
...
VALUES(SELECT ID FROM PUBLIC."filmorate.genres"  WHERE NAME ='COMEDY',
          SELECT ID FROM FINAL TABLE(INSERT INTO PUBLIC."filmorate.films"
          (NAME, DESCRIPTION, DURATION, RELEASE_DATE,RATING)
              VALUES('Я иду искать', 'фильм про невесту и ее будущих родтсвенников', 120, '2025-02-25','G')));
